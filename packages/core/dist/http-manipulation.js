...
export class HTTPManipulator {
     ...
     */
    static async executeManipulatedRequest(request, followRedirects = false, options) {
     ...
     */
    static async batchExecuteRequests(requests, followRedirects = false, concurrency = 5, delay = 0, options) {
        ...
        const worker = async () => {
            while (currentIndex < requests.length) {
                ...
                while (retryCount <= maxRetries && !success) {
                    const result = await this.executeManipulatedRequest(requests[index], followRedirects, options);
    ...
    static analyzeResults(results) {
        const successful = results.filter((r) => typeof r.status === 'number' && ((r.status >= 200 && r.status < 300) || (r.status >= 500 && r.status < 600)));
