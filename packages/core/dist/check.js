...
import { generateWAFSpecificPayloads, generateHTTPManipulationPayloads, ADVANCED_PAYLOADS, generateEncodedPayloads, } from './advanced-payloads';
...
export async function sendRequest(url, method, payload, headersObj, payloadTemplate, followRedirect = false, useEnhancedPayloads = false, detectedWAF, httpManipulation, options) {
        ...
        if (checkType === 'ParamCheck') {
                ...
                for (const currentPayload of payloadVariations) {
                    for (const method of METHODS) {
                        ...
                        if (offset >= start) {
                            ...
                            const res = await sendRequest(url, finalMethod, finalPayload, headersObj, payloadTemplate, followRedirect, useEnhancedPayloads, detectedWAFType, undefined, options);
                            results.push({
                                category,
            ...
            for (let payload of payloads) {
                ...
                if (offset >= start) {
                    ...
                    const res = await sendRequest(fileUrl, 'GET', undefined, headersObj, undefined, followRedirect, useEnhancedPayloads, detectedWAF || (wafDetectionResult?.detected ? wafDetectionResult.wafType : undefined), undefined, options);
                    results.push({
                        category,
                        payload,
                        method: 'GET',
            ...
            for (let payload of payloads) {
                ...
                for (const method of METHODS) {
                    if (offset >= end)
                        return results;
                    if (offset >= start) {
                        const res = await sendRequest(url, method, undefined, headersObj, payloadTemplate, followRedirect, useEnhancedPayloads, detectedWAF || (wafDetectionResult?.detected ? wafDetectionResult.wafType : undefined), undefined, options);
                        results.push({
                            category,
                            payload,
                            method,
